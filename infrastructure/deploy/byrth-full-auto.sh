#!/bin/bash
# =============================================================================
#  BYRTH.AI — FULL AUTOMATED DEPLOYMENT
#  One script, zero thinking. Deploys everything in the correct order.
#
#  Usage (on a fresh Ubuntu 22.04 server or VM):
#    curl -sL https://raw.githubusercontent.com/flexworx/byrth/main/infrastructure/deploy/byrth-full-auto.sh | sudo bash
#    — OR —
#    sudo bash byrth-full-auto.sh
#
#  Deployment Order:
#    Step 1: Install all development tools
#    Step 2: Clone Byrth repository
...
echo -e "${CYAN}╔══════════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║                                                          ║${NC}"
echo -e "${CYAN}║      ${BOLD}BYRTH.AI — Full Automated Deployment${NC}${CYAN}               ║${NC}"
...
# ═══════════════════════════════════════════════════════════════════════════
step "2/12" "Cloning Byrth repository"
# ═══════════════════════════════════════════════════════════════════════════
...
ok "Database: byrth_platform (user: byrth)"
...
if [ -n "$DOMAIN" ]; then
    ...
    warn "No BYRTH_DOMAIN set — skipping SSL. Set it and run: certbot --nginx -d yourdomain.com"
fi
...
systemctl enable byrth-api byrth-web >> "$LOG" 2>&1
systemctl restart byrth-api
sleep 3
systemctl restart byrth-web
sleep 2
systemctl reload nginx
...
echo -e "${GREEN}╔══════════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║                                                          ║${NC}"
echo -e "${GREEN}║          ${BOLD}BYRTH.AI IS FULLY DEPLOYED${NC}${GREEN}                    ║${NC}"
...
echo -e "${GREEN}║  1. Edit /opt/byrth/api-server/.env:                    ║${NC}"
echo -e "${GREEN}║     - Set PROXMOX_TOKEN_SECRET                          ║${NC}"
echo -e "${GREEN}║     - Set AWS_ACCESS_KEY_ID + SECRET                    ║${NC}"
echo -e "${GREEN}║  2. Set domain: export BYRTH_DOMAIN=byrth.ai            ║${NC}"
echo -e "${GREEN}║  3. Enable SSL: certbot --nginx -d byrth.ai             ║${NC}"
echo -e "${GREEN}║  4. Change admin password in dashboard                  ║${NC}"
echo -e "${GREEN}║                                                          ║${NC}"
echo -e "${GREEN}║  SERVICES:                                               ║${NC}"
echo -e "${GREEN}║    systemctl status byrth-api byrth-web nginx            ║${NC}"
echo -e "${GREEN}║    journalctl -u byrth-api -f  (view logs)              ║${NC}"
echo -e "${GREEN}║                                                          ║${NC}"
echo -e "${GREEN}║  UPDATES:                                                ║${NC}"
echo -e "${GREEN}║    cd /opt/byrth && git pull                            ║${NC}"
echo -e "${GREEN}║    npm run build                                        ║${NC}"
echo -e "${GREEN}║    systemctl restart byrth-api byrth-web                 ║${NC}"
echo -e "${GREEN}║                                                          ║${NC}"
echo -e "${GREEN}╚══════════════════════════════════════════════════════════╝${NC}"
echo ""
echo -e "Full log: $LOG"
